...
async def weather(city = 'Краснодар', weather_api = weather_api):
    ...
    weather1 = (f'{hbold(city).center(30)}\n'
          f'{hunderline(wd)} - 🌡️ {hbold(temp)}℃\n'
          f'{hunderline("Ощущается как")} - 🌡️ {hbold(feels_like)}℃\n'
          f'{hunderline("Максимальная температура")} - 🌡️ {hbold(temp_max)}℃\n'
          f'{hunderline("Минимальная температура")} - 🌡️ {hbold(temp_min)}℃\n'
